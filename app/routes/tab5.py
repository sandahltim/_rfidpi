...
def init_hand_counted_db():
    try:
        if not os.path.exists(HAND_COUNTED_DB):
            ...
            conn.commit()
            conn.close()
            logger.debug("Initialized tab5_hand_counted.db")
        os.chmod(HAND_COUNTED_DB, 0o666)
    except Exception as e:
        logger.error(f"Error initializing tab5_hand_counted.db: {e}", exc_info=True)

@tab5_bp.route("/", strict_slashes=False)
def show_tab5():
    logger.debug("Loading /tab5/ endpoint")
    init_hand_counted_db()

    try:
        with DatabaseConnection() as conn:
            logger.debug("Fetching RFID data")
        ...
        with sqlite3.connect(HAND_COUNTED_DB, timeout=10) as conn:
            logger.debug("Fetching hand-counted data")
        ...
        for contract, item_list in contract_map.items():
            logger.debug(f"Processing contract: {contract}")
        ...
        logger.debug("Sorting parent data")
        parent_data.sort(key=lambda x: x["contract"].lower())

        logger.debug("Rendering Tab 5")
        ...
        logger.error(f"Error in show_tab5: {e}", exc_info=True)
        return "Internal Server Error", 500

@tab5_bp.route("/save_hand_counted", methods=["POST"])
def save_hand_counted():
    logger.debug("Saving hand-counted entry")
    try:
        ...
        logger.error(f"Error saving hand-counted entry: {e}", exc_info=True)
        return "Internal Server Error", 500

@tab5_bp.route("/update_hand_counted", methods=["POST"])
def update_hand_counted():
    logger.debug("Updating hand-counted entry")
    try:
        common_name = request.form.get("common_name")
        last_contract_num = request.form.get("last_contract_num")
        ...
        if not last_contract_num.startswith("L"):
            logger.error(f"Invalid contract: {last_contract_num} must start with 'L'")
        ...
        with sqlite3.connect(HAND_COUNTED_DB, timeout=10) as conn:
            ...
            if not row:
                logger.error(f"No matching L contract found: {last_contract_num}, {common_name}")
            ...
            if new_total < 0:
                logger.error(f"Returned quantity {returned_qty} exceeds original {orig_total}")
        ...
        logger.error(f"Error updating hand-counted entry: {e}", exc_info=True)
        return "Internal Server Error", 500

@tab5_bp.route("/subcat_data", methods=["GET"])
def subcat_data():
    logger.debug("Hit /tab5/subcat_data endpoint")
    contract = request.args.get('contract')
    common_name = request.args.get('common_name')
    page = int(request.args.get('page', 1))
    per_page = 20

    logger.debug(f"Params - Contract: {contract}, Common Name: {common_name}, Page: {page}")

    with DatabaseConnection() as conn:
        items = get_active_rental_items(conn)
        logger.debug(f"Fetched {len(items)} active rental items")

    with sqlite3.connect(HAND_COUNTED_DB, timeout=10) as conn:
        logger.debug("Fetching hand-counted data for subcat")
        conn.row_factory = sqlite3.Row
        hand_items = conn.execute(
            "SELECT * FROM hand_counted_items WHERE last_contract_num = ? AND common_name = ?", 
            (contract, common_name)
        ).fetchall()
        logger.debug(f"Fetched {len(hand_items)} hand-counted items for {contract}, {common_name}")
    ...
    logger.debug(f"Combined items: {len(filtered_items)} before filters")
    if filter_contract:
        filtered_items = [item for item in filtered_items if filter_contract in item["last_contract_num"].lower()]
        logger.debug(f"After contract filter: {len(filtered_items)}")
    if filter_common_name:
        filtered_items = [item for item in filtered_items if filter_common_name in item.get("common_name", "").lower()]
        logger.debug(f"After common_name filter: {len(filtered_items)}")
    ...
    logger.debug(f"Returning: Total Items: {total_items}, Total Pages: {total_pages}, Current Page: {page}, Items: {len(paginated_items)}")
