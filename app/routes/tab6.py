...
def show_tab6():
    ...
    for category, item_list in category_map.items():
        ...
        on_contract = sum(1 for i in item_list if i["status"] in ["Delivered", "On Rent"])
        ...
        middle_map[category] = [
            {"common_name": name, "total": len(grp)}
            for name, grp in common_name_map.items()
        ]
...
def subcat_data():
    ...
    filtered_items = items
    if filter_common_name:
        filtered_items = [i for i in filtered_items if i.get("common_name") == filter_common_name]
    if filter_tag_id:
        filtered_items = [i for i in filtered_items if i.get("tag_id") == filter_tag_id]
    if filter_last_contract:
        filtered_items = [i for i in filtered_items if i.get("last_contract_num") == filter_last_contract]
    if filter_status:
        filtered_items = [i for i in filtered_items if i.get("status") == filter_status]
    if filter_rental_class_num:
        rental_class_nums = [num.strip() for num in filter_rental_class_num.split(',') if num.strip()]
        if not rental_class_nums:  # Full list from your data
            rental_class_nums = [
        ...
        filtered_items = [i for i in filtered_items if i.get("rental_class_num") in rental_class_nums]

    category_items = [i for i in filtered_items if categorize_item(i) == category]
    subcat_items = [i for i in category_items if i.get("common_name") == common_name] if common_name else category_items
    ...
    return jsonify({
        "items": [{
            ...
            "last_contract_num": itm.get("last_contract_num", "N/A")
